...
OAuth2.prototype.updateLocalStorage = function() {
  ...
  var data = {};
  var variables = [
    'accessToken', 'accessTokenDate', 'perms', 'clientId', 'clientSecret',
...
OAuth2.prototype.getAccessAndRefreshTokens = function(authorizationCode, callback) {
  ...
  var key = null;
  if (method == 'POST') {
    if(that.adapter.basicAuthenticationRequired()){
      var formData = "";
      ...
      xhr.setRequestHeader("Authorization", "Basic "+window.btoa(items["client_id"]+":"+items["client_secret"]))
      console.log(xhr);
      xhr.send(formData);
    }
    else{
 ...
 */
OAuth2.prototype.refreshAccessToken = function(refreshToken, callback, errorCallback) {
  var xhr = new XMLHttpRequest();
  ...
  xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  var data = this.get();
  var items = this.adapter.refreshTokenParams(refreshToken, this.getConfig());

  if(this.adapter.basicAuthenticationRequired()){
    var formData = "";
    for (var key in items) {
      if((key != "client_id") && (key != "client_secret")){
        if(formData != "") formData += "&";
...
OAuth2.loadAdapter = function(adapterName, callback) {
  ...
  script.src = chrome.extension.getURL('js/adapters/' + adapterName + '.js');
 ...
 */
OAuth2.prototype.authorize = function(callback,errorCallback) {
  var that = this;
...
const MendeleyAdapter = require("./mendeley");
const MindmeisterAdapter = require("./mindmeister");
